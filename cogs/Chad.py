...
class Chad(commands.Cog):
    ...
    async def pull_messages(self, ctx):
    ...
    async def chad_magic(self, ctx, response):
        ...
        text = await self.pull_messages(ctx)
        ...
        font = Font('/usr/share/fonts/opentype/malayalam/Gayathri-Regular.otf') #Random font that ill use just for now
        ...
        with Image(filename=template) as img: #Load the original image
            with img.clone() as clonedimage: #Clone the image for manipulation
                if template == 'data/chadtemplate.png': #If the default option gets loaded up
                    ...
                    clonedimage.caption(text[1], x+1075, y+700, width, height, font) #Add the caption response to the image
    ...
    @commands.command()
    async def chad(self, ctx, response='default'):
        await self.chad_magic(ctx, response)
        with open('data/response.png', 'rb') as response:
            await ctx.send(file=discord.File(response, 'post.png')) #This should (hopefully) send the modified image into the channel
